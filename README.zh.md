...
报告中的每个会话除体积外还会给出标题、所属项目路径、最后活跃时间和消息数量，因为会话 ID 只是一串随机字符，无法说明会话内容。项目一列显示的是绝对 worktree 路径；路径超出终端宽度时会从开头截断，让用于区分不同检出的末尾目录保持可见。这些描述信息只会为报告实际展示的会话查询，因此 `--top` 决定了它们的开销上限。
